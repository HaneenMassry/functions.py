...
def uploadLesson():
    ...
    blob = bucket.blob("Class/"+Class + "/" + Subject+"/" + Teacher + "/Video_Files/"+ filename)
    #if (blob == None):
    blob.upload_from_filename(filename)
    vidLink = blob.media_link

    #else:
       #g print("this package is found")
    blob = bucket.blob("Class/" + Class + "/" + Subject +"/" + Teacher + "/TXT_Files/"+ filename1)
    blob.upload_from_filename(filename1)
    srtLink=blob.media_link
...
#srtLink= #todo add the link of the str + video
#vidLink=""


extractAud(Video)
subtitelsSRT('my_result.mp3')
#lesson = creatLesson(Subject, Teacher, Class, Video, Srt_File,vidLink,srtLink)

uploadLesson()
#uploadLesson(lesson)


# save the subtitle srt&video file according to the input
# replace the vid
# upload the files to firebase -> libraries -> the subject name
